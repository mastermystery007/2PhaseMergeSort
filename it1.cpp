...
struct CompareTuples {
    bool operator()(Tuple const& p1, Tuple const& p2)
    {
        ...
        for(int i=0;i<col_index.size();i++)
        {
            ...
            if (!ascend){return elem1[col_index[i]] < elem2[col_index[i]];}
            else{return elem1[col_index[i]] > elem2[col_index[i]];}
        }
        return true;
    }
};
...
void do_sort_and_write(vector<vector<string>> v, int scratch_file_id)
{
        ...
        if(ascend==false){reverse(v.begin(), v.end());}
...
{ 
    ...
    bool moreinput = true;
    string line;
    int thread_size = run_size/no_of_threads;
    int i;
    ...
    while(moreinput)
    {
        ...
        if(curr_scratch_file<no_of_threads)
        {
            thread husain(do_sort_and_write,tuples,curr_scratch_file);
            th[curr_scratch_file] = std::move(husain);
        }else
        {
            thread & te = th[curr_scratch_file%no_of_threads];
            te.join();
            thread husain(do_sort_and_write,tuples,curr_scratch_file);
            th[curr_scratch_file%no_of_threads] = std::move(husain);
